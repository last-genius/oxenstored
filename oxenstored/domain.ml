...
type port_pair = {local: Xeneventchn.t; remote: int}

(* Sentinal port_pair with both set to EVTCHN_INVALID *)
let invalid_ports = {local= Xeneventchn.of_int 0; remote= 0}
...
type t = {
  ...
  ; mutable io_credit: int
        (* the rounds of ring process left to do, default is 0,
           	                           usually set to 1 when there is work detected, could
           	                           also set to n to give "lazy" clients extra credit *)
  ; mutable conflict_credit: float
        (* Must be positive to perform writes; a commit
...
let dump d chan =
  fprintf chan "dom,%d,%nd,%d,%d\n" d.id d.mfn d.ports.remote
    (Xeneventchn.to_int d.ports.local)
...
let rebind_evtchn d remote_port =
  ...
  debug "domain %d rebind %s => %s" d.id
    (string_of_port_pair d.ports)
    (string_of_port_pair new_ports) ;
  d.ports <- new_ports

let notify dom = Event.notify dom.eventchn dom.ports.local
...
let make ?local_port ~remote_port id mfn interface eventchn =
  let local =
    ...
    | None ->
        Event.bind_interdomain eventchn id remote_port
    | Some p ->
        Xeneventchn.of_int p
  in
