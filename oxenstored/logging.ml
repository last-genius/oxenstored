...
type log_destination = File of string | Syslog of Syslog.facility
...
let prefix log_destination ?level ?key date =
  match log_destination with
  | File _ ->
      let level =
        match level with Some x -> Printf.sprintf "|%5s" x | None -> ""
      in
      let key = match key with Some x -> "|" ^ x | None -> "" in
...
let log_rotate ref_ch log_file log_nb_files =
  ...
  let log_files =
    let rec aux accu n =
      if n >= log_nb_files then
        accu
      else if n = 1 && Sys.file_exists log_file then
        aux [(log_file, 1)] 2
      else
...
let make_file_logger log_file log_nb_files log_nb_lines log_nb_chars post_rotate
  ...
  let stop () = try flush !channel ; close_out !channel with _ -> () in
  let restart () =
    stop () ;
    channel := open_out_gen [Open_append; Open_creat] 0o644 log_file
  in
  let rotate () =
  ...
  let write ?level:_ s =
    ...
    flush !channel ;
    incr counter ;
    if !counter > log_nb_lines then rotate ()
  in
  {stop; restart; rotate; write}
...
let make_syslog_logger facility =
  ...
  let () =
    let tz_is_set =
      try String.length (Unix.getenv "TZ") > 0 with Not_found -> false
    in
    if not tz_is_set then Unix.putenv "TZ" "/etc/localtime"
  in
  let nothing () = () in
  let write ?level s =
    let level =
    ...
    in
    (* Syslog handles the date and level internally *)
    Syslog.log facility level s
  in
  {stop= nothing; restart= nothing; rotate= nothing; write}
...
let set_xenstored_logger logger =
  ...
  logger.write ~level:Info
    (Printf.sprintf "Xen Storage Daemon, version %d.%d" Define.xenstored_major
       Define.xenstored_minor
    )

let init_xenstored_log () =
  match !xenstored_log_destination with
  | File file ->
      if !xenstored_log_level <> Null && !xenstored_log_nb_files > 0 then
        let logger =
          make_file_logger file !xenstored_log_nb_files !xenstored_log_nb_lines
            !xenstored_log_nb_chars ignore
        in
...
let string_of_tid ~con tid =
  if tid = 0 then
    ...
    sprintf "%-12s" (sprintf "%s.%i" con tid)
...
let access_log_destination =
  ref (File (Paths.xen_log_dir ^ "/xenstored-access.log"))
...
let init_access_log post_rotate =
  match !access_log_destination with
  | File file ->
      if !access_log_nb_files > 0 then
        let logger =
          make_file_logger file !access_log_nb_files !access_log_nb_lines
            !access_log_nb_chars post_rotate
        in
...
let read_coalesce ~tid ~con data =
  if !access_log_read_ops then
    access_logging Coalesce ~tid ~con ~data:("read " ^ data) ~level:Debug

let write_coalesce data =
  access_logging Coalesce ~data:("write " ^ data) ~level:Debug
...
let xb_op ~tid ~con ~ty data =
  let print =
    match ty with
    | Xenbus.Xb.Op.Read | Xenbus.Xb.Op.Directory | Xenbus.Xb.Op.Getperms ->
        !access_log_read_ops
...
let start_transaction ~tid ~con =
  if !access_log_transaction_ops && tid <> 0 then
    access_logging ~tid ~con (XbOp Xenbus.Xb.Op.Transaction_start) ~level:Debug

let end_transaction ~tid ~con =
  if !access_log_transaction_ops && tid <> 0 then
    access_logging ~tid ~con (XbOp Xenbus.Xb.Op.Transaction_end) ~level:Debug
...
let fallback_exception_handler exn bt =
  ...
  *)
  ...
  match !xenstored_logger with
  | Some l ->
      error "xenstored-fallback" "%s" msg ;
      l.stop ()
  | None -> (
