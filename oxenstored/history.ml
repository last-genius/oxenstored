...
let trim ?txn () =
  ...
  history :=
    match Transaction.oldest_short_running_transaction () with
    | None ->
        [] (* We have no open transaction, so no history is needed *)
    | Some (_, txn) ->
        (* keep records with finish_count recent enough to be relevant *)
        List.filter
          (fun r -> r.finish_count > txn.Transaction.start_count)
          !history
...
let push (x : history_record) =
  ...
  match dom with
  | None ->
      () (* treat socket connections as always free to conflict *)
  | Some d ->
      if not (Domain.is_free_to_conflict d) then history := x :: !history
...
let filter_connections ~ignore ~since ~f =
  ...
  List.fold_left
    (fun acc hist_rec ->
      if
        hist_rec.finish_count > since
        && (not (hist_rec.con == ignore))
        && (not (Hashtbl.mem acc hist_rec.con))
        && f hist_rec
      then
        Hashtbl.replace acc hist_rec.con () ;
      acc
    )
