...
let do_argv () =
  ...
  let speclist =
    [
      ( "--no-domain-init"
      , Arg.Unit (fun () -> domain_init := false)
  ...
  in
  let usage_msg =
    "usage : xenstored [--config-file <filename>] [--no-domain-init] [--help] \
     [--no-fork] [--reraise-top-level] [--restart] [--disable-socket]"
  in
