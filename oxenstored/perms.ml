...
let char_of_permty perm =
  match perm with READ -> 'r' | WRITE -> 'w' | RDWR -> 'b' | NONE -> 'n'
...
let permty_of_char c =
  ...
  | 'n' ->
      NONE
  | _ ->
      invalid_arg "unknown permission type"

(* node permissions *)
module Node = struct
  type t = {owner: int; other: permty; acl: (int * permty) list}

  let create owner other acl = {owner; other; acl}
...
module Connection = struct
  ...
  let full_rights : t = {main= (0, [READ; WRITE]); target= None}

  let create ?(perms = [NONE]) domid : t = {main= (domid, perms); target= None}
  ...
  let is_owner (connection : t) id =
    ...
    | Some target ->
        fst connection.main = id || fst target = id
    | None ->
        fst connection.main = id

  let is_dom0 (connection : t) = is_owner connection 0

  let elt_to_string (i, p) =
    Printf.sprintf "%i%S" i
      (String.concat "" (List.map String.of_char (List.map char_of_permty p)))

  let to_string connection =
    Printf.sprintf "%s%s"
      (elt_to_string connection.main)
      (default "" (may elt_to_string connection.target))
end
...
let check_owner (connection : Connection.t) (node : Node.t) =
  if !activate && not (Connection.is_dom0 connection) then
    Connection.is_owner connection (Node.get_owner node)
  else
    true
...
let lacks (connection : Connection.t) request (node : Node.t) =
  let check_acl domainid =
    let perm =
      if List.mem_assoc domainid (Node.get_acl node) then
        List.assoc domainid (Node.get_acl node)
      else
        Node.get_other node
    in
...
let check connection request node =
  if lacks connection request node then
...
let can_fire_watch connection perms =
  (not !watch_activate) || List.exists (has connection READ) perms
