...
module Node = struct
  ...
  let set_value node nvalue =
    if node.value = nvalue then
      node
    else
      {node with value= nvalue}
  ...
  let del_all_children node = {node with children= SymbolMap.empty}
  ...
  let check_owner node connection =
    if not (Perms.check_owner connection node.perms) then (
      Logging.info "store|node" "Permission denied: Domain %d not owner"
        (get_owner node) ;
      ...
      let node =
        {
          node with
          children=
            SymbolMap.map walk node.children |> SymbolMap.filter is_valid
        }
      in
      match f node with
      | Some keep ->
          keep
      | None ->
          {node with perms= {node.perms with owner= invalid}}
    in
    walk
...
module Path = struct
  ...
  let of_path_and_name path name =
    match (path, name) with [], "" -> [] | _ -> path @ [name]
  ...
  let get_hierarchy path = Utils.get_hierarchy path
  ...
  let rec lookup_get node path =
    ...
    | [] ->
        raise Define.Invalid_path
    | h :: [] -> (
      try Node.find node h with Not_found -> raise Define.Doesnt_exist
    )
    | h :: l ->
...
let path_write store perm path value =
  ...
  let do_write node name =
    ...
    with Not_found ->
      node_created := true ;
      Node.check_perm node perm Perms.WRITE ;
      Node.add_child node (Node.create name node.Node.perms value)
  in
  if path = [] then (
...
let path_rm store perm path =
  let do_rm node name =
    ...
    with Not_found -> raise Define.Doesnt_exist
  in
  if path = [] then (
    Node.check_perm store.root perm Perms.WRITE ;
...
(* accessing functions *)
let get_node store path = Path.get_node store.root path
...
let traversal root_node f =
  let rec _traversal path node =
    f path node ;
    let node_path =
      Path.of_path_and_name path (Symbol.to_string node.Node.name)
    in
...
let dump_store_buf root_node =
  ...
  let dump_node path node =
    ...
    Printf.bprintf buf "%s/%s{%s}" pathstr
      (Symbol.to_string node.Node.name)
...
let write store perm path value =
  ...
  else (* Check the domain entries limit too *)
...
let mkdir store perm path =
  ...
  if not (existing || Perms.Connection.is_dom0 perm) then
...
let rm store perm path =
  ...
  | Some rmed_node ->
      ...
      store.quota <-
        Node.recurse
          (fun node quota -> Quota.del_entry quota (Node.get_owner node))
          rmed_node store.quota
...
let stats store =
  ...
  traversal store.root (fun _path _node -> incr nb_nodes) ;
