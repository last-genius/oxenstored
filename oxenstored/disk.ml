...
let string_unescaped s =
  ...
  let read_escape () =
    ...
    | c ->
        if is_digit c then (
          let v =
            (undec s.[!i] * 100) + (undec s.[!i + 1] * 10) + undec s.[!i + 2]
          in
          i := !i + 2 ;
          Char.chr v
        ) else
  ...
  while !i < len do
    let c = match s.[!i] with '\\' -> read_escape () | c -> c in
    Buffer.add_char d c ; incr i
  done ;
  Buffer.contents d
...
let rec map_string_list_range l s =
  ...
  | [] ->
      []
  | (a, b) :: l ->
      String.sub s a (b - a) :: map_string_list_range l s
...
let rec parse_perm s =
  ...
  if len = 0 then
    ...
    let x = String.sub s 0 !i and lx = String.sub s !i len in
    x :: parse_perm lx
...
let read store =
  ...
  let parse_line s =
    let path, perm, value =
      ...
      if String.contains s '=' then
        let ss =
          map_string_list_range [(0, pi); (pi + 1, epi); (si + 2, len)] s
        in
        (List.nth ss 0, List.nth ss 1, List.nth ss 2)
      else
        let ss = map_string_list_range [(0, pi); (pi + 1, epi)] s in
  ...
  with exc -> error "caught exn %s" (Printexc.to_string exc)
...
let write store =
  if !enable then
    try
      ...
      let channel =
        open_out_gen [Open_wronly; Open_creat; Open_trunc] 0o600 tfile
      in
