...
let to_string quota domid =
  try
    Printf.sprintf "dom%i quota: %i/%i" domid
      (DomidMap.find domid quota.cur)
      quota.maxent
  with Not_found -> Printf.sprintf "dom%i quota: not set" domid

let create () = {maxent= !maxent; maxsize= !maxsize; cur= DomidMap.empty}
...
let update_entry quota_cur id diff =
  ...
  if nb = 0 then
    DomidMap.remove id quota_cur
  else
    DomidMap.add id nb quota_cur

let del_entry quota id = {quota with cur= update_entry quota.cur id (-1)}

let add_entry quota id = {quota with cur= update_entry quota.cur id 1}
...
let merge orig_quota mod_quota dest_quota =
  let fold_merge id nb dest =
    ...
    | 0 ->
        dest (* not modified *)
    | diff ->
        update_entry dest id diff (* update with [x=x+diff] *)
  in
