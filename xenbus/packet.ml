...
let create tid rid ty data = {tid; rid; ty; data}
...
let to_string pkt =
  let header =
    string_of_header pkt.tid pkt.rid (Op.to_cval pkt.ty) (String.length pkt.data)
  in
  header ^ pkt.data

let unpack pkt = (pkt.tid, pkt.rid, pkt.ty, pkt.data)
