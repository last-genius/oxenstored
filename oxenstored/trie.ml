...
module Node = struct
  type 'a t = {key: string; value: 'a option; children: 'a t StringMap.t}

  let _create key value = {key; value= Some value; children= StringMap.empty}

  let empty key = {key; value= None; children= StringMap.empty}
...
let mem_node nodes key = StringMap.mem key nodes

let find_node nodes key = StringMap.find key nodes
...
let rec iter f tree =
  let aux key node = f key node.Node.value ; iter f node.Node.children in
  ...
  | h :: t ->
      if mem_node tree h then
        ...
        if t = [] then
          ...
          sub_node node.Node.children t
      else
        raise Not_found

let sub tree path =
  try (sub_node tree path).Node.children with Not_found -> StringMap.empty
...
let rec unset tree = function
  ...
  | h :: t ->
      if mem_node tree h then
        ...
        let new_node =
          if t = [] then
            Node.set_children (Node.empty h) children
          else
            Node.set_children node children
        in
        if StringMap.is_empty children && new_node.Node.value = None then
          remove_node tree h
        else
          replace_node tree h new_node
      else
        raise Not_found
