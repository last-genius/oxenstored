...
let trim_start lc s =
  ...
  while !i < len && List.mem s.[!i] lc do
    incr i
  done ;
...
let parse_line stream =
  ...
  let to_config s =
    ...
    | [k; v] ->
        Some (trim_end lc k, trim_start lc v)
    | _ ->
        None
  in
  ...
  List.iter
    (fun (k, v) ->
      let parse ~err_msg parser v f =
        match parser v with None -> append (k, err_msg) | Some r -> f r
      in
      try
        if not (List.mem_assoc k expected) then
          other k v
        else
          let ty = List.assoc k expected in
          match ty with
          | Unit f ->
              f ()
          | Bool f ->
              parse ~err_msg:"expect bool arg" bool_of_string_opt v f
          | String f ->
              f v
          | Int f ->
              parse ~err_msg:"expect int arg" int_of_string_opt v f
          | Float f ->
              parse ~err_msg:"expect float arg" float_of_string_opt v f
          | Set_bool r ->
              parse ~err_msg:"expect bool arg" bool_of_string_opt v (fun x ->
                  r := x
              )
          | Set_string r ->
              r := v
          | Set_int r ->
              parse ~err_msg:"expect int arg" int_of_string_opt v (fun x ->
                  r := x
              )
          | Set_float r ->
              parse ~err_msg:"expect float arg" float_of_string_opt v (fun x ->
                  r := x
              )
      with
      | Not_found ->
          append (k, "unknown key")
      | exn ->
