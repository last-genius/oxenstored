  ...
  let permdiff =
    List.fold_left
      (fun acc path ->
        ...
        | Some n1, Some n2 ->
            (not
               (Perms.equiv (Store.Node.get_perms n1) (Store.Node.get_perms n2))
            )
            || acc
        | _ ->
            true || acc
...
let get_lowest path1 path2 =
  ...
  | None ->
      Some path1
  | Some path2 ->
      Some (Store.Path.get_common_prefix path1 path2)
...
let test_coalesce oldroot currentroot optpath =
  ...
  | Some path -> (
      ...
      | None, None -> (
          ...
          let pnode =
            Store.Path.get_node currentroot (Store.Path.get_parent path)
          in
          match pnode with
          | None ->
              false (* ok it doesn't exists, just bail out. *)
          | Some _ ->
              true
        )
      | _ ->
...
let oldest_short_running_transaction () =
  let rec last = function [] -> None | [x] -> Some x | _ :: xs -> last xs in
  last !short_running_txns
...
let trim_short_running_transactions txn =
  ...
  let keep =
    match txn with
    | None -> (
        function start_time, _ -> start_time >= cutoff
      )
    | Some t -> (
        function start_time, tx -> start_time >= cutoff && tx != t
      )
  in
  short_running_txns := List.filter keep !short_running_txns
...
let commit ~con t =
  ...
  let has_commited =
    ...
    | No ->
        true
    | Full (_id, oldstore, cstore) ->
        (* "cstore" meaning current canonical store *)
        let commit_partial oldroot cstore store =
          ...
          if
            can_coalesce oldroot (Store.get_root cstore) t.read_lowpath
            && can_coalesce oldroot (Store.get_root cstore) t.write_lowpath
          then (
            maybe
              (fun p ->
        ...
        let try_commit oldroot cstore store =
          if oldroot == Store.get_root cstore then (
            ...
            true
          ) else (* we try a partial commit if possible *)
  ...
  if has_commited && has_write_ops then
    Disk.write t.store ;
  if not has_commited then
    Logging.conflict ~tid:(get_id t) ~con
  else if not !has_coalesced then
    Logging.commit ~tid:(get_id t) ~con ;
  has_commited
